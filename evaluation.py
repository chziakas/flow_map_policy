...
def evaluate(
    ...
    Args:
        ...
        benchmark: If True, time each sample_actions call and report latency stats.
                   Should be used with JAX_PLATFORMS=cpu and jax.disable_jit().
    ...
    ctx = jax.disable_jit() if benchmark else nullcontext()
    with ctx:
        call_count = 0
    ...
    if benchmark and inference_latencies:
        latencies = np.array(inference_latencies)
        stats['inference_ms_mean'] = float(np.mean(latencies))
        stats['inference_ms_std'] = float(np.std(latencies))
        stats['inference_ms_median'] = float(np.median(latencies))
        stats['inference_num_calls'] = len(latencies)
        print(f'\nInference benchmark: {np.mean(latencies):.1f} +/- {np.std(latencies):.1f} ms/action '
              f'(median {np.median(latencies):.1f} ms, {len(latencies)} calls after {warmup_calls} warmup)')
